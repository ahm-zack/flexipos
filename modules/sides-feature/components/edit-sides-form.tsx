...
import { useUpdateSide } from "../hooks/use-sides";
import { uploadMenuImage } from "@/lib/image-upload";
import type { SideOrder, UpdateSideOrder, Modifier } from "@/lib/schemas";
import { ModifierManager } from "@/components/modifier-manager";

interface EditSidesFormProps {
  side: SideOrder | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function EditSidesForm({
  side,
  open,
  onOpenChange,
}: EditSidesFormProps) {
  const [formData, setFormData] = useState<UpdateSideOrder>({
    nameAr: "",
    nameEn: "",
    imageUrl: "",
    priceWithVat: "",
  ...
  useEffect(() => {
    if (side) {
      setFormData({
        ...
        priceWithVat: side.priceWithVat,
        modifiers: side.modifiers || [],
      });
      setPreviewUrl(side.imageUrl || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        const uploadedUrl = await uploadMenuImage(selectedFile, "sides");
        if (uploadedUrl) {
          imageUrl = uploadedUrl;
        } else {
          toast.warning(
            "Image upload failed, but side will be updated with existing data"
          );
        }
      }
      const sideData: UpdateSideOrder = { ...formData, imageUrl };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the details for &quot;{side.nameEn}&quot;.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g., بطاطس"
                dir="rtl"
              />
            </div>
            {/* Image Upload */}
            <div className="space-y-2">
              <Label htmlFor="editSidesImageFile">Side Image (Optional)</Label>
              <div className="space-y-3">
                <Input
                  id="editSidesImageFile"
                  type="file"
                  accept="image/*"
                ...
                {(previewUrl || formData.imageUrl) && (
                  <div className="relative w-full h-48 border-2 border-dashed border-gray-200 rounded-lg overflow-hidden">
                    <Image
                      src={
                        previewUrl ||
                        getReliableImageUrl(formData.imageUrl || "", "sides")
                      }
                      alt="Side preview"
                      fill
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={updateSideMutation.isPending || isUploading}
            >
              {isUploading
                ? "Updating..."
                : updateSideMutation.isPending
                ? "Saving..."
                : "Save Changes"}
            </Button>
          </DialogFooter>
