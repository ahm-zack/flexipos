...
import { useCreateSide } from "../hooks/use-sides";
import { uploadMenuImage } from "@/lib/image-upload";
import type { CreateSideOrder, Modifier } from "@/lib/schemas";
import { ModifierManager } from "@/components/modifier-manager";

interface CreateSidesFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function CreateSidesForm({ open, onOpenChange }: CreateSidesFormProps) {
  const [formData, setFormData] = useState<CreateSideOrder>({
    nameAr: "",
    nameEn: "",
    imageUrl: "",
    priceWithVat: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createSideMutation.mutateAsync({
        ...formData,
        imageUrl,
      ...
      toast.success("Side created successfully! 🍟", {
        description: "The side will appear in the list automatically",
      });
      ...
      toast.error("Failed to create side");
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new side to your menu. All fields marked with * are required.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            ...
            {/* Image Upload */}
            <div className="space-y-2">
              <Label htmlFor="createSidesImageFile">
                Side Image (Optional)
              </Label>
              <div className="space-y-3">
                <Input
                  id="createSidesImageFile"
                  type="file"
                  accept="image/*"
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={createSideMutation.isPending || isUploading}
