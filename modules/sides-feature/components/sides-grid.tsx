...
import { SidesManagementCard } from "./sides-management-card";
import { SideOrder } from "@/lib/db/schema";
import { SidesCashierCard } from "./sides-cashier-card";

interface SidesGridProps {
  sides: SideOrder[];
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">
          Try adjusting your search or add some sides to get started.
        </p>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {sides.map((side) => {
        if (!showActions && showCartActions) {
          return <SidesCashierCard key={side.id} side={side} />;
        } else {
          return (
            <SidesManagementCard
              key={side.id}
