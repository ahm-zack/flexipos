...
export function BeverageCashierView() {
  const [searchTerm, setSearchTerm] = useState("");
  const { data: beverages, isLoading, error } = useBeverages("cashier");
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-center mb-6">
            Discover our refreshing beverage selection
          </p>
          {/* Search Bar */}
        ...
        {/* Beverage Grid - Cashier View (No management actions) */}
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredBeverages.map((beverage) => (
              <BeverageCashierCard key={beverage.id} beverage={beverage} />
            ))}
          </div>
        )}
      </div>
