...
import { useCreateBeverage } from "../hooks/use-beverages";
import { uploadMenuImage } from "@/lib/image-upload";
import type { Modifier, CreateBeverage } from "@/lib/schemas";
import { ModifierManager } from "@/components/modifier-manager";

interface CreateBeveragesFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function CreateBeveragesForm({
  open,
  onOpenChange,
}: CreateBeveragesFormProps) {
  const [formData, setFormData] = useState<CreateBeverage>({
    nameEn: "",
    nameAr: "",
    imageUrl: "",
    priceWithVat: "",
  ...
  const createBeverageMutation = useCreateBeverage();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createBeverageMutation.mutateAsync({
        ...formData,
        imageUrl,
      ...
      toast.success("Beverage created successfully! 🥤", {
        description: "The beverage will appear in the list automatically",
      });
      ...
      toast.error("Failed to create beverage");
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new beverage to your menu. All fields marked with * are
            required.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            ...
            {/* Image Upload */}
            <div className="space-y-2">
              <Label htmlFor="createBeverageImageFile">
                Beverage Image (Optional)
              </Label>
              <div className="space-y-3">
                <Input
                  id="createBeverageImageFile"
                  type="file"
                  accept="image/*"
                ...
                {(previewUrl || formData.imageUrl) && (
                  <div className="relative w-full h-48 border-2 border-dashed border-gray-200 rounded-lg overflow-hidden">
                    <Image
                      src={
                        previewUrl ||
                        getReliableImageUrl(formData.imageUrl, "beverages")
                      }
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={createBeverageMutation.isPending || isUploading}
            ...
            >
              ...
              ) : createBeverageMutation.isPending ? (
                "Creating..."
              ) : (
                "Create Beverage"
              )}
