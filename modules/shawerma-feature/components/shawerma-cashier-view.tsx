...
import { useShawarmas } from "../hooks/use-shawermas";

export function ShawermaCashierView() {
  const [searchTerm, setSearchTerm] = useState("");
  const { data: shawarmas, isLoading, error } = useShawarmas("cashier");
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-center mb-6">
            Discover our delicious shawarma selection
          </p>
          {/* Search Bar */}
        ...
        {/* Shawarma Grid - Cashier View (No management actions) */}
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredShawarmas.map((shawarma) => (
              <ShawermaCashierCard key={shawarma.id} shawarma={shawarma} />
            ))}
          </div>
        )}
      </div>
