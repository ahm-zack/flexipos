...
import { useUpdateAppetizer } from "../hooks/use-appetizers";
import { uploadMenuImage } from "@/lib/image-upload";
import type { Appetizer, UpdateAppetizer, Modifier } from "@/lib/schemas";
import { ModifierManager } from "@/components/modifier-manager";

interface EditAppetizerFormProps {
  appetizer: Appetizer | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function EditAppetizerForm({
  appetizer,
  open,
  onOpenChange,
}: EditAppetizerFormProps) {
  const [formData, setFormData] = useState<UpdateAppetizer>({
    nameAr: "",
    nameEn: "",
    imageUrl: "",
    priceWithVat: "",
  ...
  const updateAppetizerMutation = useUpdateAppetizer();
  ...
  useEffect(() => {
    if (appetizer) {
      setFormData({
        ...
        priceWithVat: appetizer.priceWithVat,
        modifiers: appetizer.modifiers || [],
      });
      setPreviewUrl(appetizer.imageUrl || "");
    ...
    try {
      ...
      if (selectedFile) {
        ...
        if (uploadedUrl) {
          ...
          toast.warning(
            "Image upload failed, but appetizer will be updated with existing data"
          );
        }
      }
      const appetizerData: UpdateAppetizer = { ...formData, imageUrl };
      await updateAppetizerMutation.mutateAsync({
        id: appetizer.id,
        data: appetizerData,
      });
      toast.success("Appetizer updated successfully!");
      setSelectedFile(null);
      onOpenChange(false);
    } catch (error) {
      toast.error("Failed to update appetizer");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Appetizer</DialogTitle>
          <DialogDescription>
            Update the details for &quot;{appetizer.nameEn}&quot;.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            ...
            {/* Image Upload */}
            <div className="space-y-2">
              <Label htmlFor="editAppetizerImageFile">
                Appetizer Image (Optional)
              </Label>
              <div className="space-y-3">
                <Input
                  id="editAppetizerImageFile"
                  type="file"
                  accept="image/*"
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={updateAppetizerMutation.isPending || isUploading}
            >
              {isUploading
                ? "Updating..."
                : updateAppetizerMutation.isPending
                ? "Saving..."
                : "Save Changes"}
            </Button>
          </DialogFooter>
