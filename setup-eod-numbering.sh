...
# Run the EOD report numbering migration directly
echo "Applying EOD report numbering migration..."

# Get the Supabase URL and key from environment or config
SUPABASE_URL=$(grep 'url' supabase/config.toml | cut -d'"' -f4)
SUPABASE_ANON_KEY=$(grep 'anon_key' supabase/config.toml | cut -d'"' -f4)

# Read the migration file
MIGRATION_SQL=$(cat supabase/migrations/20250706210000_add_eod_report_numbering.sql)
...
echo "Migration SQL created. Now applying..."

# Apply the migration using psql if available
if command -v psql &> /dev/null; then
    echo "Using psql to apply migration..."
    # This would need the actual database connection string
    # For now, let's just show the SQL that needs to be executed
    echo "Please run the following SQL on your database:"
    cat temp_migration.sql
else
    echo "psql not available. Please run the following SQL on your database:"
    cat temp_migration.sql
fi
